...
pub fn sort_columns(
    ...
    if cols.is_empty() || cols[0].is_empty() { return; }
    let num_rows = cols[0].len();
    ctx.span(SpanOperation::Sort { rows: num_rows }, |_ctx| {
        let mut row_order: Vec<usize> = (0..num_rows).collect();
        let order_positions: Vec<(usize, OrderDirection)> = order_by.iter().filter_map(|spec| {
