...
use engine_simple::execute::{self, ExecutionContext};
...
fn schema_for(name: &str, cols: &[(&str, DataType, bool)]) -> (TableSchema, Schema) {
    ...
    let query_schema = Schema::new(
        cols.iter().map(|(n, _, _)| ColumnDef {
            table: Some(name.into()), name: (*n).into(),
        }).collect(),
    );
...
impl TestDb {
    fn new() -> Self { Self { tables: HashMap::new(), schemas: HashMap::new() } }
...
fn parent_duration_gte_children_sum() {
    ...
    let plan = planner::plan(&ast, &db.schemas).unwrap();
    let mut ctx = ExecutionContext::new();
