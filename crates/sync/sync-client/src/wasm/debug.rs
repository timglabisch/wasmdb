//! `#[wasm_bindgen]` exports for the debug toolbar. All non-generic —
//! they read the type-erased client through `with_client_dyn` and the
//! pure-Rust counters in `wasmdb-debug`.
...
pub fn debug_sync_status() -> Result<JsValue, JsError> {
    with_client_dyn(|client| {
        ...
        let status = SyncStatus {
            ...
            streams: detail
                .iter()
                .map(|(id, entries)| StreamInfo {
                    id: id.0,
                    pending_count: entries.len(),
                    is_idle: entries.is_empty(),
                    pending: entries
                        .iter()
                        .map(|(seq, zset_len)| PendingDetail {
                            seq_no: *seq,
...
pub fn debug_subscriptions() -> Result<JsValue, JsError> {
    ...
    with_client_dyn(|client| {
        ...
        let debug = SubscriptionDebug {
            count,
            subscriptions: sub_tables
                .iter()
                .map(|(id, tables)| SubInfo {
                    id: *id,
                    sql: rdb
                        .subscription_sql(SubscriptionId(*id))
                        .unwrap_or("")
                        .to_string(),
                    tables: tables.clone(),
        ...
        serde_wasm_bindgen::to_value(&debug).map_err(|e| JsError::new(&e.to_string()))
    })
}
        ...
        fn estimate_table_memory(t: &sql_engine::storage::Table) -> usize {
            ...
            for col in &t.columns {
                bytes += match col {
                    ...
                    TypedColumn::NullableStr { values, .. } => {
                        values.iter().map(|s| s.len() + 24).sum::<usize>() + values.len() / 8
                    }
                    TypedColumn::NullableUuid { values, .. } => {
                        values.len() * 16 + values.len() / 8
                    }
                };
            }
            ...
            let tables = names
                ...
                .map(|name| {
                    ...
                    TableInfo {
                        ...
                        fragmentation_ratio: if physical > 0 {
                            deleted as f64 / physical as f64
                        } else {
                            0.0
                        },
                        columns: t
...
pub fn debug_table_rows(table_name: &str, _db_kind: &str, limit: usize) -> Result<JsValue, JsError> {
    with_client_dyn(|client| {
        ...
        let rows: Vec<Vec<CellValue>> = table
            .row_ids()
            .take(limit)
            .map(|row_id| (0..col_count).map(|col| table.get(row_id, col)).collect())
            .collect();
