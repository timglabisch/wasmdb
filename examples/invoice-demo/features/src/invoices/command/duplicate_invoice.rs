...
impl Command for DuplicateInvoice {
    fn execute_optimistic(&self, db: &mut Database) -> Result<ZSet, CommandError> {
        ...
        let numbers = read_str_col(db, "SELECT invoices.number FROM invoices WHERE invoices.id = :id",
        ...
        let doc_type        = str_field!("doc_type");
        ...
        let mut acc = execute_stmt(db, sql!(
            "INSERT INTO invoices (id, customer_id, number, status, date_issued, date_due, notes, doc_type, parent_id, service_date, cash_allowance_pct, cash_allowance_days, discount_pct, payment_method, sepa_mandate_id, currency, language, project_ref, external_id, billing_street, billing_zip, billing_city, billing_country, shipping_street, shipping_zip, shipping_city, shipping_country) \
             VALUES ({id}, {customer_id}, {number}, 'draft', {date_issued}, {date_due}, {notes}, {doc_type}, {parent_id}, {service_date}, {cash_allowance_pct}, {cash_allowance_days}, {discount_pct}, {payment_method}, {sepa_mandate_id}, {currency}, {language}, {project_ref}, {external_id}, {billing_street}, {billing_zip}, {billing_city}, {billing_country}, {shipping_street}, {shipping_zip}, {shipping_city}, {shipping_country})",
            id = new_id,
        ...
        acc.extend(execute_stmt(db, sql!(
            "INSERT INTO activity_log (id, timestamp, entity_type, entity_id, action, actor, detail) \
             VALUES ({activity_id}, {timestamp}, 'invoice', {entity_id}, 'duplicate_from', 'demo', {detail})",
