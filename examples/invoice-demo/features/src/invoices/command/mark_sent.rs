...
impl Command for MarkSent {
    fn execute_optimistic(&self, db: &mut Database) -> Result<ZSet, CommandError> {
        ...
        let mut acc = execute_sql(
            db,
            "UPDATE invoices SET status = 'sent' WHERE invoices.id = :id",
            Params::from([p_uuid("id", &self.id)]),
        )?;
        ...
        acc.extend(execute_sql(
            ...
            "INSERT INTO activity_log (id, timestamp, entity_type, entity_id, action, actor, detail) \
             VALUES (:aid, :ts, 'invoice', :id, 'status_sent', 'demo', :detail)",
            Params::from([
    ...
    impl ServerCommand for MarkSent {
        async fn execute_server(
            ...
            invoice_entity::Entity::update_many()
                .col_expr(invoice_entity::Column::Status, "sent".to_string().into())
