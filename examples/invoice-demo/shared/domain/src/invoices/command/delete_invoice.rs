...
impl Command for DeleteInvoice {
    fn execute_optimistic(
        ...
        let mut acc = sql!("DELETE FROM payments WHERE invoice_id = {self.id}").execute(db)?;
        acc.extend(sql!("DELETE FROM positions WHERE invoice_id = {self.id}").execute(db)?);
        acc.extend(sql!("DELETE FROM invoices WHERE id = {self.id}").execute(db)?);
        acc.extend(
            sql!(
